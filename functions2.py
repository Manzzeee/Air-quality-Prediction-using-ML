...
def predict2(x_val,x,y):
    ...
    x=featureNormalize(x)
    y=featureNormalize(y)
#save the Regression classifier
# serialize model to learning rate
    reg = Regression()
    reg.set_learning_rate(0.001)
    reg.set_max_iterations(10000)
    reg.set_l1_penalty(0.1)
    ...
    return(y_val)
#save the NormalEquation 
# serialize model to transpose
