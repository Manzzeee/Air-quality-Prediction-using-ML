...
# Definding the input layer and the first hidden layer
def c1(state,crop,year):
    ...
    return(l)
# Definding the input layer and the Second hidden layer
...
def c2(state,crop):
    ...
    title="Prediction of " + crop + " in " + state
    N=len(ye)
    ind=np.arange(N)
    width=0.5
    [fig, ax]=plt.subplots()
    ...
    plt.close('all')
    return(fig)
# Definding the input layer and the  maxrow layer
...
def c3(crop,year):
    ...
    l.remove(l2)
    l.insert(0,l2)
    title="Prediction of " + crop + " in " + str(year)
    N=len(yi)
    ind=np.arange(N)
    width=0.5
    [fig, ax]=plt.subplots()
...
def c4(state,year):
    ...
    l.remove(l2)
    l.insert(0,l2)
    title="Prediction in " + state + " in " + str(year)
    N=len(yi)
    ind=np.arange(N)
    width=0.5
    [fig, ax]=plt.subplots()
...
def c5(state,crop,year):
    ...
    yearlist=gyears
    [yieldpredict1,yieldfig]=predict('year','prediction',year,yearlist,yieldlist)
